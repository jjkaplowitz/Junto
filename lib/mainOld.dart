...
class DataTableState extends State<DataTable> {
  ...
  void _loadRecommend() async {
    ...
    String data = await Getdata(url);
    List<String> _listofind = data.split("|");
    List<List<dynamic>> _listFinal = [];
    final _rawData = await rootBundle.loadString("reddit_questions_long.csv");
    const conv = CsvToListConverter(fieldDelimiter: ';');
    List<List<dynamic>> _listData = conv.convert(_rawData);
    for (int i = 0; i < _listofind.length - 1; i++) {
      _listFinal.add(_listData[int.parse(_listofind[i])]);
    }
    setState(() {
      _data = _listFinal;
    });
